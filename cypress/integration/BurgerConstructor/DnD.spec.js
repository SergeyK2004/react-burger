describe('Создание бургера перетаскиванием ингредиента и оформление заказа', function () {
  beforeEach(function () {
  ...
  it('Перетаскиваем булку и оформляем заказ', function () {
    ...
    cy.get('[class^=burger-constructor]')
      ...
      .should('not.exist');
    cy.get('[class^=Ingredient_card_]').first().trigger('dragstart');
    cy.get('[class^=BurgerConstructor_list_]').trigger('drop');
    cy.get('[class^=burger-constructor]')
      .contains('Краторная булка')
      .should('exist');
